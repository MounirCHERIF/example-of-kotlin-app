...
class ParksCardFragment : Fragment(R.layout.fragment_card), OnMapReadyCallback{
    private lateinit var mMap: GoogleMap
    private lateinit var marker_loc:LatLng
    ...
    fun loadData(){
        ...
        supportMapFragment.requireView().setVisibility(View.INVISIBLE);
        ...
        CoroutineScope(Dispatchers.IO+ exceptionHandler).launch {
            ...
            withContext(Dispatchers.Main) {
                if (response.isSuccessful && response.body() != null) {
                    ...
                    vm_parking.data.clear()
                    vm_parking.data.addAll(parkingModel.data)

                    for(parking in parkingModel.data){
                        marker_loc = LatLng(parking.latitude.toDouble(), parking.longitude.toDouble())
                        mMap.addMarker(MarkerOptions().position(marker_loc).title(parking.nomParking))
                        parking.etat = "Fermé"
                    ...
                    loadingPB!!.visibility = View.INVISIBLE
                    supportMapFragment.requireView().setVisibility(View.VISIBLE);
                    val current_LatLon = LatLng(loc.latitude,loc.longitude)
                    mMap.moveCamera(CameraUpdateFactory.newLatLng(current_LatLon))
                    mMap.addMarker(MarkerOptions().position(current_LatLon).title("Ma position"))
                        ?.setIcon(
